...
app.post('/catalog/sync', async (req, res) => {
    await syncTikTokCatalogEngine();
    res.json({ status: 'ok', message: 'Catalogue vidéo TikTok synchronisé avec succès' });
});

// 14. MOTEUR AUTONOME DE SYNCHRONISATION DU CATALOGUE VIDÉO TIKTOK (100% SANS ACTION MANUELLE)
const getTikTokClientCredentials = () => {
    let clientKey = process.env.TIKTOK_CLIENT_KEY;
        ...
        if (fs.existsSync(envPath)) {
            ...
            content.split('\n').forEach(line => {
                ...
                if (parts.length >= 2) {
                    ...
                    if (k === 'TIKTOK_CLIENT_KEY') clientKey = v;
                    if (k === 'TIKTOK_CLIENT_SECRET') clientSecret = v;
                }
            });
        }
    }
...
const syncTikTokCatalogEngine = async () => {
    try {
        console.log('[TikTok Catalog Daemon] 🔄 Vérification autonome des vidéos publiées sur TikTok...');
        const userRef = db.collection('users').doc(TARGET_FIRESTORE_USER);
        const docSnap = await userRef.get();
        if (!docSnap.exists) return;

        const authData = docSnap.data().tiktokAuth;
        if (!authData || !authData.refreshToken) {
            console.warn('[TikTok Catalog Daemon] Aucun refreshToken disponible.');
            return;
        }

        const { clientKey, clientSecret } = getTikTokClientCredentials();
        if (!clientKey || !clientSecret) {
            console.warn('[TikTok Catalog Daemon] Clés client TikTok manquantes.');
            return;
        }

        // Renouvellement automatique du token OAuth (valide 24h)
        ...
        if (!refreshData.access_token) {
            console.warn('[TikTok Catalog Daemon] Échec rafraîchissement token:', refreshData.message || refreshData.error);
            return;
        }
        ...
        // Récupération paginée de l'ensemble des vidéos du créateur
        let allVideos = [];
        let hasMore = true;
        let cursor = 0;
        let pageCount = 0;
        ...
        while (hasMore && pageCount < 20) {
            ...
            if (vJson.data && vJson.data.videos) {
                const batch = vJson.data.videos.map(v => ({
                    ...
                    shareUrl: v.share_url || `https://www.tiktok.com/@${TIKTOK_USERNAME}/video/${v.id}`
                }));
        ...
        if (allVideos.length > 0) {
            ...
            console.log(`[TikTok Catalog Daemon] ✅ Catalogue vidéo mis à jour (${allVideos.length} vidéos, dernière: "${allVideos[0].title.slice(0, 30)}...")`);
        }
    } catch (err) {
        console.warn('[TikTok Catalog Daemon] Erreur synchronisation catalogue:', err.message);
    }
};

// 15. DÉMARRAGE DU SERVEUR SUR LE PORT CONFIGURÉ
...
server.listen(PORT, () => {
    ...
    // Démarrage initial de la boucle de détection du Live
    startLiveTracker();

    // Démarrage initial de la synchronisation du catalogue après 5s, puis toutes les heures (1h = 3600s)
    setTimeout(syncTikTokCatalogEngine, 5000);
    setInterval(syncTikTokCatalogEngine, 3600000);
